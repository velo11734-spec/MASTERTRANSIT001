...
export default function BankingInfoStep() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push('/en/company/onboarding/step-5');
    } catch (error: any) {
      console.error('Step 4 Error:', error);
      alert(error.message || 'Failed to save banking info');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <section className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-black text-white p-4">
      <h1 className="text-4xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 to-pink-400">Company Onboarding – Step 4</h1>
      <form onSubmit={handleSubmit} className="w-full max-w-md space-y-4 bg-gray-800 rounded-xl p-6 shadow-lg">
        <div>
          ...
          <input id="bankName" type="text" required value={bankName} onChange={e => setBankName(e.target.value)} className="w-full rounded-md bg-gray-700 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 px-3 py-2" />
        </div>
        <div>
          <label className="block text-sm font-medium mb-1" htmlFor="accountNumber">Account Number</label>
          <input id="accountNumber" type="text" required value={accountNumber} onChange={e => setAccountNumber(e.target.value)} className="w-full rounded-md bg-gray-700 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 px-3 py-2" />
        </div>
